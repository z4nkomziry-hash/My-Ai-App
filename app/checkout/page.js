...
const methods = [
  { id: 'FIB', name: 'First Iraqi Bank (FIB)', iconType: 'building', value: '+964 750 604 5491', link: 'tel:+9647506045491', label: 'Phone Number', color: 'from-blue-600 to-blue-400' },
  { id: 'FastPay', name: 'FastPay Wallet', iconType: 'phone', value: '+964 750 604 5491', link: 'tel:+9647506045491', label: 'Phone Number', color: 'from-green-600 to-green-400' },
  { id: 'USDT', name: 'USDT (TRC20)', iconType: 'bitcoin', value: 'TKUfVwnjyT2KUa9xnBreT32YLLJEwACHpc', link: null, label: 'Wallet Address', color: 'from-orange-600 to-yellow-400', warning: 'Send only USDT on TRC20 network' }
];
...
function MethodIcon({ type, className }) {
  switch (type) {
    case 'building': return <Building2 className={className || 'w-6 h-6'} />;
    case 'phone': return <Phone className={className || 'w-6 h-6'} />;
    case 'bitcoin': return <Bitcoin className={className || 'w-6 h-6'} />;
    default: return <Building2 className={className || 'w-6 h-6'} />;
  }
}

function getCheckoutNotes(lang) {
  const val = getTranslation(lang, 'checkout.notes');
...
export default function CheckoutPage() {
  ...
  const current = methods.find(function(m) { return m.id === method; }) || methods[0];
  const checkoutNotes = getCheckoutNotes(lang);
  ...
  function handleCopy(text) {
    ...
    setCopied(text);
    toast.success(t('checkout.copied') || 'Copied!');
