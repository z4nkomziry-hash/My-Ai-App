...
export async function submitPaymentRequest(paymentData) {
  ...
  try {
    const { data, error } = await supabase.from('payment_requests').insert({
      user_id: paymentData.user_id, method: paymentData.method,
      transaction_id: paymentData.transaction_id, receipt_url: paymentData.receipt_url || '',
      full_name: paymentData.full_name, email: paymentData.email,
...
export async function updatePaymentStatus(paymentId, status, adminNotes = '') {
  ...
  try {
    const { data, error } = await supabaseAdmin.from('payment_requests').update({
      status: status, admin_notes: adminNotes, updated_at: new Date().toISOString()
...
export async function approveUserPro(userId) {
  ...
  try {
    const { data, error } = await supabaseAdmin.from('profiles').update({
      subscription_status: 'pro', updated_at: new Date().toISOString()
    }).eq('user_id', userId).select().single();
    if (error) throw error;
    return data;
  } catch { return null; }
 ...
 * Uses OTP (6-digit code) verification instead of email links
 */
 ...
 * This is the key function — it uses type: 'signup' which works with 6-digit codes
 */
 ...
 * Uses type: 'signup' to resend the verification code
 */
 ...
 * Sends a 6-digit code to the email for login
 */
...
/**
 * Verify OTP for sign-in (passwordless login)
 */
